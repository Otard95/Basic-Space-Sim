...
app.get('/*', function(req, res) {
  ...
  var is_valid = app.validate_path(req.path);

  // if no file spesified in path, set default
  var final_path = app.get_final_path(req.path);
...
app.validate_path = function(path) {
...
app.get_final_path = function(path) {

  var path_parts = path.split('/');
  if (path_parts[(path_parts.length - 1)].indexOf('.') == -1) {
    return app.path_settings.base_dir + path + app.path_settings.default_filename;
  } else {
    return app.path_settings.base_dir + path;
  }
